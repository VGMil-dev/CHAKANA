...
async function markOrderPaid(session: Record<string, any>, eventId: string) {
  ...
  const amountCents = (session.amount_total as number | null) ?? order.total_cents;
...
async function markOrderFailed(paymentIntent: Record<string, any>, eventId: string) {
  const supabase = getSupabaseAdmin();
  ...
  const { error: paymentError } = await supabase.from('payments').upsert({
    ...
    stripe_event_id: eventId,
    stripe_payment_id: paymentIntentId,
    amount_cents: (paymentIntent.amount as number | null) ?? order.total_cents,
    status: 'failed',
  });
