...
async function run() {
  ...
  const commerceTables = ['users', 'merchants', 'tambus', 'products', 'orders', 'payments'] as const;
  ...
  results.push(await check('Cliente autenticado puede leer sus ordenes', async () => {
    ...
    assert(!signInError, `No se pudo iniciar sesion de QA: ${signInError?.message}`);
    ...
    assert(!error, `No se pudo consultar ordenes: ${error?.message}`);
  ...
  if (env.commerceProductId) {
    results.push(await check('Producto de comercio de QA existe', async () => {
      ...
      assert(data.active === true, 'Producto QA deberia estar activo');
    }));
  }
